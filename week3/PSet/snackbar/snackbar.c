...
float get_cost(string item)
{
    for (int i = 0; i < NUM_ITEMS; i++)
    {
        if (strcasecmp(item, menu[i].item) == 0) //comparing two string case insensitive
        {
            return menu[i].price;  //returning price of item
        }
    }
    printf("Invalid Item!\n"); // In case invalid input will print "Invalid Item" and teturn 0
    return 0.0;
}
